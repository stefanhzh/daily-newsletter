#!/usr/bin/env python3
"""Latent Space ingestion adapter."""
...
class LatentSpaceAdapter(BaseAdapter):
    source_id = "latent-space"
    api_url = "https://www.latent.space/api/v1/posts?limit=30"
    ...
    def _row_to_item(self, row: dict[str, object]) -> IngestedItem | None:
        if row.get("audience") not in {None, "everyone"}:
            return None
        ...
        return IngestedItem(
            ...
            byline="Latent Space",
            section="newsletter",
            discovery_method="latent_space_substack_api",
            body_text=body_text,
            fulltext_note="body_from_substack_api",
        )
